...
ВНЕСЁННЫЕ ИЗМЕНЕНИЯ:
1. МАКСИМАЛЬНО АГРЕССИВНАЯ АЛЛОКАЦИЯ для достижения 1.0% маржинальности в день:
   - avail >= 25: до 80% доступной маржи (очень много маржи)
   - avail >= 15: до 90% доступной маржи (много маржи)
   - avail >= 8: до 95% доступной маржи (средняя маржа)
   - avail >= 4: до 98% доступной маржи (мало маржи)
   - avail >= 2: до 99% доступной маржи (минимальная маржа)
   - avail < 2: вход невозможен

2. УВЕЛИЧЕНИЕ БАЗОВОГО РАЗМЕРА на 50% для роста доходности

3. СНИЖЕНИЕ МИНИМАЛЬНЫХ ПОРОГОВ:
   - min_quote: 60% вместо 80% от equity
   - avail: 1.5 вместо 4.0 USDT

4. ГИБРИДНЫЙ ПОРЯДОК ВХОДА: SPOT_FIRST при избытке маржи, PERP_FIRST при ограничениях

5. PERP_FIRST по умолчанию: экономия маржи, меньше ошибок "Insufficient balance"

6. УЛУЧШЕННЫЕ ПРОВЕРКИ МАРЖИ: учёт availableBalance + free + лимиты

7. СТАБИЛИЗАЦИЯ КАПА: буфер 15% вместо 1% для предотвращения резких колебаний

8. УМНОЕ РАСПРЕДЕЛЕНИЕ: не тратит всю маржу в одну пару, не простаивает при избытке

ЦЕЛЬ: повышение дневной маржинальности с 0.15% до 1.0% (рост в 6.7 раза)
"""
    ...
    while True:
        try:
            ...
            for sym in symbols_order:
                ...
                # СНИЖЕННЫЙ ПОРОГ для максимизации входов: 60% вместо 80% от equity
                if min_quote > 0 and min_quote > eq * 0.6:
                    dlog(f"{now_s()} [{sym}] min_quote≈{min_quote:.2f} USDT > 60% equity≈{eq:.2f}, skip")
                    continue
                ...
                # УВЕЛИЧЕНИЕ БАЗОВОГО РАЗМЕРА на 50% для достижения 1.0% маржинальности
                base_alloc = max(scaled_alloc * 1.5, min_quote)
                
                # МАКСИМАЛЬНО АГРЕССИВНАЯ АЛЛОКАЦИЯ для достижения 1.0% маржинальности в день
                if avail >= 25.0:
                    # Очень много маржи - используем до 80% доступной
                    eff_alloc = min(base_alloc, avail * 0.8)
                elif avail >= 15.0:
                    # Много маржи - используем до 90% доступной
                    eff_alloc = min(base_alloc, avail * 0.9)
                elif avail >= 8.0:
                    # Средняя маржа - используем до 95% доступной
                    eff_alloc = min(base_alloc, avail * 0.95)
                elif avail >= 4.0:
                    # Мало маржи - используем до 98% доступной
                    eff_alloc = min(base_alloc, avail * 0.98)
                elif avail >= 2.0:
                    # Минимальная маржа - используем до 99% доступной
                    eff_alloc = min(base_alloc, avail * 0.99)
                else:
                ...
                can_enter = (
                    ...
                    and (avail >= 1.5)  # СНИЖЕННЫЙ ПОРОГ: 1.5 вместо 4.0 для максимизации входов
                )
                # cooldown
                ...
                if can_enter and not is_marked_open(con, sym) and not_in_cooldown:
                    try:
                        ...
                        # ГИБРИДНЫЙ ПОРЯДОК ВХОДА: PERP_FIRST для экономии, SPOT_FIRST для скорости
                        if avail >= 20.0:
                            order = "SPOT_FIRST"  # Быстрый вход при избытке маржи
                        else:
                            order = "PERP_FIRST"  # Экономия при ограниченной марже
